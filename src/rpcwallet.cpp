...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    if (nNodeMode == NT_FULL)
    {
        ...
        obj.push_back(Pair("gameunitsxsupply", ValueFromAmount(pindexBest->nAnonSupply)));
    }
...
Value getnewextaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Returns a new GAMEUNITS ext address for receiving payments."
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid GAMEUNITS address: ")+s.name_);
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    if (!sxAddr.SetEncoded(sEncoded))
        throw std::runtime_error("Invalid GAMEUNITS stealth address.");
...
Value reloadanondata(const Array& params, bool fHelp)
{
    ...
    // check from 500, once anon transactions started
    while (pindex->nHeight < (fTestNet ? 0 : 500) && pindex->pnext)
        pindex = pindex->pnext;
